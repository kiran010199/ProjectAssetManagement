...
function MyForm(props){
    ...
    useEffect(()=>{
        ...
        axios
        .get(`http://localhost:3001/asset/${props.id}`)
        .then(
            response =>{
    ...
    function handleSubmit(event){
            ...
            //send info to server
            axios
            .put(`http://localhost:3001/asset/${props.id}`,inputs)
            .then(response =>{
                console.log('promise fullfilled')
                console.log(response)
                alert("The asset details are updated")
            })
        }
    return(
        <>
        <h1 className="centerfooter">Update Asset</h1>
